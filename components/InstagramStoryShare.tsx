...
const Preview = forwardRef<PreviewRef,PreviewProps>(
  ...
  return (
      ...
      >
      {/* Bully 로고 */}
      <div className="absolute top-16 right-16 text-white text-5xl font-bold">
        Bully
      </div>

      {/* 작성자 이름 */}
      <div className="text-white text-6xl font-bold text-center mb-10">
        {authorName}
      </div>
      ...
      <div className="flex justify-center text-[rgba(255,215,0,0.9)] items-center gap-1 h-[112px] px-16 rounded-2xl">
        {hashtags.map((has, index)=>{
          return (
            <div key={`has-${index}`} className="text-5xl font-bold text-center">
              {has}
            </div>
          )
        })}
