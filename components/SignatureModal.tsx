...
interface SignatureModalProps {
  ...
  isOpen: boolean;
  onClose: () => void;
  onLikeChange?: (signatureId: string, newLikeCount: number, isLiked: boolean) => void;
}

const SignatureModal = ({ signatureId, isOpen, onClose, onLikeChange }: SignatureModalProps) => {
  ...
  // useRef를 단일 객체로 변경
  const previewRef = useRef<{ download: (fileName: string) => Promise<void> } | null>(null);
  ...
  const handleDownload = async () => {
    if (previewRef.current) {
      ...
      if (author_name && signature_url) {
        await previewRef.current.download(getFileName(author_name, signature_url));
      }
    }
  };
  ...
  useEffect(() => {
    ...
    async function loadSignatureData() {
      ...
      try {
        ...
        // 게시글 정보 가져오기
        const { data: signatureData, error: signatureError } = await getSignatureById(signatureId);
  ...
  const handleLike = async () => {
    try {
      ...
      if (newLikedState) {
        ...
        const { data: updatedSignature, error: updateError } = await supabase.rpc('increment_likes', {
          signature_id: signatureId
        });
        ...
        // signatures 테이블의 likes 컬럼 업데이트
        const { data: updatedSignature, error: updateError } = await supabase.rpc('decrement_likes', {
          signature_id: signatureId
        });
      ...
      if (refreshedSignature) {
        ...
        // 부모 컴포넌트에 알림
        if (onLikeChange) {
          onLikeChange(signatureId, refreshedSignature.likes || 0, newLikedState);
        }
      }
    } catch (err) {
  ...
  return (
    <div 
      className="fixed inset-0 bg-black bg-opacity-70 z-50 flex items-center justify-center z-50 overflow-y-auto"
        ...
        {loading ? (
          ...
          <div className="p-6">
            ...
            <div className="flex items-center mb-4">
              <div className="w-12 h-12 rounded-full overflow-hidden mr-3 rounded-full bg-white">
                {/* <Image
                  src={signature.profile_image || '/profiles/k1.gif'}
                  alt="Profile"
                  width={48}
                  height={48}
              ...
              <div>
                <p className="text-white font-medium">{signature.author_name || 'Anonymous'}</p>
            ...
            {signature.signature_url && (
              <div className="relative w-full aspect-square bg-transparent mb-4 overflow-hidden">
                <img
            ...
            {/* 메시지 */}
            <div className="mb-4">
              <p className="text-white text-lg break-all">{signature.message}</p>
            </div>
            ...
            <div className="mb-4">
              ...
              >
                ...
                <svg className='w-4 h-4 md:w-6 md:h-6' fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                  <path
            ...
            {/* 댓글 목록 */}
            <div className="mb-6">
              <h3 className="text-lg font-semibold text-white mb-4">댓글 {comments.length}개</h3>
              
              {comments.length === 0 ? (
                <p className="text-gray-400 text-center py-4">첫 댓글을 남겨보세요!</p>
              ) : (
                <div className="space-y-4">
                  {comments.map((comment) => (
                    <div key={comment.id} className="bg-black border-1 border border-kanye-accent text-white rounded-lg p-3">
                      ...
                      <p className="text-white mt-1 break-all">{comment.message}</p>
                    </div>
                  ))}
                </div>
              )}
            ...
            <form onSubmit={handleSubmitComment} className="space-y-3">
              ...
              <div className="flex">
                <input
                  ...
                  className=" flex-1 bg-black border-1 border border-kanye-accent text-white px-4 py-2 rounded-l-lg focus:outline-none"
                  required
                />
                <button
                  type="submit"
                  disabled={commentLoading}
                  className="bg-kanye-accent text-white px-4 py-2 rounded-r-lg disabled:opacity-50"
                >
          ...
          <div className="p-6 text-center text-gray-400">게시글을 찾을 수 없습니다.</div>
        )}
      </div>
    </div>
  );
